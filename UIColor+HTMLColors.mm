...
+ (UIColor *)anem_colorWithCSS:(NSString *)cssColor
{
    UIColor *color = nil;
    NSScanner *scanner = [NSScanner scannerWithString:cssColor];
    [scanner anem_scanCSSColor:&color];
    return (scanner.isAtEnd) ? color : nil;
}

+ (UIColor *)anem_colorWithHexString:(NSString *)hexColor
{
    UIColor *color = nil;
    NSScanner *scanner = [NSScanner scannerWithString:hexColor];
    [scanner anem_scanHexColor:&color];
    return (scanner.isAtEnd) ? color : nil;
}

+ (UIColor *)anem_colorWithRGBString:(NSString *)rgbColor
{
    UIColor *color = nil;
    NSScanner *scanner = [NSScanner scannerWithString:rgbColor];
    [scanner anem_scanRGBColor:&color];
    return (scanner.isAtEnd) ? color : nil;
}

+ (UIColor *)anem_colorWithHSLString:(NSString *)hslColor
{
    UIColor *color = nil;
    NSScanner *scanner = [NSScanner scannerWithString:hslColor];
    [scanner anem_scanHSLColor:&color];
    return (scanner.isAtEnd) ? color : nil;
}

+ (UIColor *)anem_colorWithW3CNamedColor:(NSString *)namedColor
{
    UIColor *color = nil;
    NSScanner *scanner = [NSScanner scannerWithString:namedColor];
    [scanner anem_scanW3CNamedColor:&color];
    return (scanner.isAtEnd) ? color : nil;
}
...
- (BOOL)anem_scanCSSColor:(UIColor **)color
{
    return [self anem_scanHexColor:color]
        || [self anem_scanRGBColor:color]
        || [self anem_scanHSLColor:color]
        || [self cmr_scanTransparent:color]
        || [self anem_scanW3CNamedColor:color];
}

- (BOOL)anem_scanRGBColor:(UIColor *__autoreleasing *)color
{
...
- (BOOL)anem_scanHSLColor:(UIColor *__autoreleasing *)color
{
    return [self cmr_caseInsensitiveWithCleanup:^BOOL{
...
- (BOOL)anem_scanHexColor:(UIColor *__autoreleasing *)color
{
...
- (BOOL)anem_scanW3CNamedColor:(UIColor *__autoreleasing *)color
{
    return [self cmr_caseInsensitiveWithCleanup:^BOOL{
        ...
        for (NSString *name in colorNames) {
            if ([self scanString:name intoString:NULL]) {
                if (color) {
                    *color = [UIColor anem_colorWithHexString:namedColors[name]];
                }
                return YES;
            }
        }
...
- (BOOL)cmr_scanHexTriple:(UIColor *__autoreleasing *)color
{
    NSString *hex = nil;
